---
title: "Darpan documentation"
description: "Technical product, API, backend, release, and engineering docs for Darpan."
---

Darpan is a reconciliation workspace for technical operators. It connects source systems, describes source data with schemas, runs comparisons, applies RuleSets, and preserves the output needed to explain each difference.

Read these docs in the same order a run moves through the product: prepare setup records, execute the comparison, review generated output, then adjust the source data, schema, or rules when the result points to a problem.

<Card title="Start with the quickstart" icon="play" href="/quickstart" horizontal>
  Follow one complete path from environment selection to result review.
</Card>

## Product path

<Columns cols={2}>
  <Card title="Ask Darpan navigation" icon="sparkles" href="/getting-started/ask-darpan-navigation">
    Open pages, workflows, setup records, saved runs, and generated outputs from one search surface.
  </Card>
  <Card title="Connections" icon="plug" href="/guides/connect-sftp">
    Store SFTP, NetSuite, and provider configuration that runs can reuse.
  </Card>
  <Card title="Schemas" icon="brackets-curly" href="/guides/create-schema">
    Define source data shapes before a run depends on the fields.
  </Card>
  <Card title="Run reconciliation" icon="shuffle" href="/guides/run-reconciliation">
    Execute a saved or newly configured comparison.
  </Card>
  <Card title="Review results" icon="table" href="/guides/review-results">
    Interpret generated differences and choose the next action.
  </Card>
  <Card title="API reference" icon="terminal" href="/api-reference/overview">
    Call the JSON-RPC services behind setup, execution, and review.
  </Card>
</Columns>

## For builders

The engineering docs explain how the product surface maps to the backend engine: Moqui service contracts, tenant-scoped data access, Spark comparison, Drools RuleSets, and generated output.

<Columns cols={2}>
  <Card title="Backend engine" icon="server" href="/backend/overview">
    Understand the runtime architecture behind Darpan runs.
  </Card>
  <Card title="Engineering workflow" icon="code" href="/engineering/overview">
    Follow the repo boundaries and verification model used for Darpan development.
  </Card>
  <Card title="Release updates" icon="tag" href="/releases/updates">
    Review shipped tags and their major changes.
  </Card>
  <Card title="Troubleshooting" icon="wrench" href="/troubleshooting/login-and-session">
    Diagnose login, upload, connection, and run failures.
  </Card>
</Columns>
