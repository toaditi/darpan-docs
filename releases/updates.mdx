...
## v1.5.0 — August 26, 2026

Cut together with app `v2.6.0`, `darpan-hotwax v0.8.0`, `shopify-darpan v0.6.0`, and `database-darpan v0.2.0`. NetSuite (`v0.2.0`) is unchanged.
...
- **Scheduled runs do that re-check too.** An automation and a hand-started run over the same window finally report the same numbers — **which means the difference counts your automations report will fall, sometimes sharply; see Before you deploy**.
...
- Scheduled-run verification is on by default and needs no configuration. The kill switch is `-Ddarpan.reconciliation.automation.verifyMissingDiffs=false`, and only an exact `false` disables it, so an unrecognised value leaves verification on. Verification adds roughly 46 seconds to a scheduled run.
...
- 20 methods added and one response field removed, so the contract version moves from `2` to `3`. The contract moves from 90 to 110 methods (86 public facade, 24 administrative).
...
- Scheduled runs now verify their differences, which they never did before. The counts your automations report will drop, and the drop can be large — on one production automation it went from 532 to 2.
...
- Tell whoever reads those counts before the deploy rather than after, because a count falling by two orders of magnitude overnight reads as a broken reconciliation unless it is expected.

**Upgrade**

- Existing environments load 54 seed records via `./gradlew loadDarpanUpgradeData` — the cross-tenant sharing catalog, the chat-provider catalog, the database and three new endpoint connectors with their rules-board field lists, and the one-time migration registry.
- Fifty-one are additions. Three restate rows that already exist: the stored `OMS` and `SHOPIFY` names become "HotWax" and "Shopify" wherever a system is named, and HotWax transfer orders group under HotWax in the source picker. Renaming affects future runs only — a run's file labels are recorded when it executes, so past runs keep the names they were given.
- Until the load runs, no configuration can be shared, no Slack space can be saved, databases and the new endpoints do not appear as sources, and the rules board offers no field pills.
- Then run `run#PendingMigrations` once, or use the new Migrations screen: it walks six registered one-time migrations, skips any already applied, and is safe to run again. That covers the `v1.2.0` and `v1.3.0` one-time services, so an environment behind on either no longer needs to chase them separately.
- `v1.4.0`'s upgrade data is a prerequisite.
- No schema migration is required; new tables and fields are created on startup.

**Rollback**

- Schema and seed additions are additive and the three restatements only correct display names and a picker grouping, so rolling the backend to `v1.4.0` is safe and the loaded records do not need reverting — `v1.4.0` ignores what it does not know.
- Keep the app on `v2.5.0` if the backend goes back, since `v2.6.0` depends on methods `v1.4.0` does not expose.
- An integration reading `googleChatWebhookUrl` keeps working on `v1.4.0`, which returns both that field and the masked one; an integration that moved to `webhookUrl` or `webhookConfigured` will not, as those are new in `v1.5.0`.
