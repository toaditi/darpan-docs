...
description: "How RuleSets, rules, compare scopes, and JSON schemas fit into the backend."
---

Rules and schemas are backend-owned contracts. The UI creates and edits them through facade services, but the backend validates, stores, compiles, and executes them.
...
Facade services create saved runs and RuleSet structures together when the product workflow needs a runnable compare.
...
The key service responsibilities are:
...
Schemas describe source data shape and parsing expectations. The backend stores schema definitions as records, not as UI-only configuration.
...
Workflows that depend on rules or schemas are backed by service contracts in the Darpan component. Rules and schemas are persisted backend configuration, not browser-local state.
