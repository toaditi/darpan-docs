...
Run backend checks from the Darpan workspace root unless a command says otherwise.

```text
/Users/aditipatel/sandbox/darpan-master
```
...
For docs-only backend updates, validate the docs site from the docs repo:
...
Docs validation does not prove backend behavior. Use the Gradle checks above for backend code changes.
